...
def classify_is_human(text, bar=0.5000):
    ...
    # print(predict_res)
    if predict_res[0] == 'Human' and predict_res[2] == 'Human':
        return (predict_res[1] + predict_res[3]) / 2 >= bar
    if predict_res[0] == 'ChatGPT' and predict_res[2] == 'ChatGPT':
        return (predict_res[1] + predict_res[3]) / 2 < bar
    if predict_res[0] == 'Human':
        return predict_res[1] >= predict_res[3]
    else:
        return predict_res[1] < predict_res[3]
...
if __name__ == '__main__':
    print(predict_en(text))
    print(predict_en(json_1['human_answers'][0]))
    print(predict_en(json_1['chatgpt_answers'][0]))
    print(classify_is_human(text))
    print(classify_is_human(json_1['human_answers'][0]))
    print(classify_is_human(json_1['chatgpt_answers'][0]))
