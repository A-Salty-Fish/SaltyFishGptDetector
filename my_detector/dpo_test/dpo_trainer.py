# https://github.com/WatchTower-Liu/DPO_trainer/blob/master/DPO_trl.py
...
def test_dataSet(tokenizer, data):
    ...
    for P, A, R in zip(prompt, accept, reject):
        tokenize_data = tokenize_batch_element(P, chosen=A, rejected=R, truncation_mode="keep_start",
                                               tokenizer=tokenizer, max_length=2048, max_prompt_length=2048)
...
def load_model(config, train_args):
    policy = AutoModelForCausalLM.from_pretrained(config.model.name_or_path, local_files_only=True,
                                                  torch_dtype=torch.bfloat16, trust_remote_code=True,
                                                  cache_dir=config.local_dirs)
    ref_model = AutoModelForCausalLM.from_pretrained(config.model.name_or_path, local_files_only=True,
                                                     torch_dtype=torch.bfloat16, trust_remote_code=True,
                                                     cache_dir=config.local_dirs)
    tokenizer = AutoTokenizer.from_pretrained(config.model.name_or_path, use_fast=False, local_files_only=True,
                                              trust_remote_code=True, cache_dir=config.local_dirs)
...
def main():
    ...
    print(train_args)
    trainer = DPOTrainer(policy, ref_model, args=train_args, train_dataset=dataset,
