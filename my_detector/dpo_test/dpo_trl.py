...
def load_model(config, train_args):
    policy = AutoModelForCausalLM.from_pretrained(config.model.name_or_path, torch_dtype=torch.bfloat16,
                                                  trust_remote_code=True)
    ref_model = AutoModelForCausalLM.from_pretrained(config.model.name_or_path, torch_dtype=torch.bfloat16,
                                                     trust_remote_code=True)
    tokenizer = AutoTokenizer.from_pretrained(config.model.name_or_path, use_fast=False, trust_remote_code=True)
