...
def truncate_m4(raw_m4_path, truncated_m4_path, truncate_length=256, drop_length = 128):
    for file in os.listdir(raw_m4_path):
        if file.find('.jsonl') == -1:
            continue
        if file.find('bloomz') != -1:
            continue

        with open(raw_m4_path + file, 'r', encoding='utf-8') as f, open(truncated_m4_path + file, 'w', encoding='utf-8') as out_f:
            for line in f:
                try:
                    json_obj = json.loads(line)
                    ...
                    json_obj['human_text'] = truncate_content(human_text, truncate_length=truncate_length, drop_length=drop_length)
                    json_obj['machine_text'] = truncate_content(machine_text, truncate_length=truncate_length, drop_length=drop_length)
...
if __name__ == '__main__':
    ...
    truncate_m4('./test_data/m4/', './test_data_truncated/m4/', 128, 64)
